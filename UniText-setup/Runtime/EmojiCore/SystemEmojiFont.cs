...
namespace LightSide
{
    ...
    public static class SystemEmojiFont
    {
        ...
        {
    #if UNITY_ANDROID && !UNITY_EDITOR
            var fontFromMatcher = GetEmojiFromFontMatcher();
            if (fontFromMatcher != null)
                return fontFromMatcher;
        ...
        private static extern IntPtr AFontMatcher_create();

        [DllImport("android", CallingConvention = CallingConvention.Cdecl)]
        private static extern void AFontMatcher_destroy(IntPtr matcher);

        [DllImport("android", CallingConvention = CallingConvention.Cdecl)]
        private static extern void AFontMatcher_setLocales(IntPtr matcher, string languageTags);

        [DllImport("android", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr AFontMatcher_match(
            IntPtr matcher, string familyName,
            [In] ushort[] text, uint textLength, IntPtr runLengthOut);

        [DllImport("android", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr AFont_getFontFilePath(IntPtr font);

        [DllImport("android", CallingConvention = CallingConvention.Cdecl)]
        private static extern void AFont_close(IntPtr font);

        static string GetEmojiFromFontMatcher()
        {
            try
            {
                IntPtr matcher = AFontMatcher_create();
                if (matcher == IntPtr.Zero) return null;

                try
                {
                    AFontMatcher_setLocales(matcher, "und-Zsye");

                    ushort[] text = { 0xD83D, 0xDE00 };

                    IntPtr font = AFontMatcher_match(
                        matcher, "sans-serif", text, 2, IntPtr.Zero);

                    if (font == IntPtr.Zero) return null;

                    try
                    {
                        IntPtr pathPtr = AFont_getFontFilePath(font);
                        if (pathPtr == IntPtr.Zero) return null;

                        string path = Marshal.PtrToStringAnsi(pathPtr);
                        Cat.Meow($"[SystemEmojiFont] AFontMatcher resolved: {path}");
                        return path;
                    }
                    finally
                    {
            ...
            {
                Cat.MeowWarn($"[SystemEmojiFont] AFontMatcher failed: {e.Message}");
                return null;
            }
        }
