    ...
    internal class COLRv1Renderer : IDisposable
    {
            ...
            out float bearingX, out float bearingY, out float renderScale)
        {
            ...
            if (FT.GetColorGlyphClipBox(face, glyphIndex, out var clipBox))
            {
                ...
                float maxClipDim = Math.Max(clipWidth, clipHeight);
                if (maxClipDim > pixelSize)
                {
                    fitScale = pixelSize / maxClipDim;
                }

                renderWidth = (int)Math.Ceiling(clipWidth * fitScale);
                renderHeight = (int)Math.Ceiling(clipHeight * fitScale);
                if (renderWidth < 1) renderWidth = pixelSize;
                if (renderHeight < 1) renderHeight = pixelSize;

                offsetX = -minX * fitScale;
                offsetY = -minY * fitScale;
            }
            else
            {
                bearingX = 0;
            ...
            try
            {
                ...
                BL.ContextScale(blCtx, fitScale, -fitScale);
                if (glyphOffsetX != 0 || glyphOffsetY != 0)
                {
                    BL.ContextTranslate(blCtx, glyphOffsetX / fitScale, glyphOffsetY / fitScale);
                }

                ResetTransform();
                pendingGlyphPath = IntPtr.Zero;
                ApplyTranslate(0, renderHeight);
                ApplyScale(fitScale, -fitScale);
                if (glyphOffsetX != 0 || glyphOffsetY != 0)
                {
                    ApplyTranslate(glyphOffsetX / fitScale, glyphOffsetY / fitScale);
                }
