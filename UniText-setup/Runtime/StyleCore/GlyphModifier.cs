...
namespace LightSide
{
    ...
    public abstract class GlyphModifier<T> : BaseModifier where T : unmanaged
    {
        ...
        private Action cachedOnShaped;

        protected sealed override void OnEnable()
        {
            buffers.PrepareAttribute(ref attribute, AttributeKey);

            cachedOnGlyph ??= GetOnGlyphCallback();
            uniText.MeshGenerator.onGlyph += cachedOnGlyph;

            cachedOnShaped ??= GetOnShapedCallback();
            if (cachedOnShaped != null)
                uniText.TextProcessor.Shaped += cachedOnShaped;
        }
