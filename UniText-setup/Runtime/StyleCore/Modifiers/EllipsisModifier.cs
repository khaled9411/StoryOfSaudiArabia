...
namespace LightSide
{
    ...
    [ParameterField(0, "Position", "float(0,1)", "1")]
    public class EllipsisModifier : BaseModifier
    {
        ...
        protected override void OnEnable()
        {
            ...
            uniText.BeforeGenerateMesh += InjectEllipsisDots;
        ...
        private float MeasureEllipsisWidth()
        {
            ...
            var dotFontId = fontProvider.FindFontForCodepoint('.');
            var dotFont = fontProvider.GetFontAsset(dotFontId);
            if (dotFont == null)
                return 0f;

            var fontSize = uniText.CurrentFontSize;
            if (Shaper.TryGetGlyphInfo(dotFont, '.', fontSize, out _, out var advance))
                return advance * 3;
        ...
        private void InjectEllipsisDots()
        {
            ...
            {
                for (var i = 0; i < lineTruncations.Count; i++)
                    InjectDotsAtCluster(lineTruncations[i].ellipsisCluster, fontProvider);
            }
            ...
            for (var r = 0; r < ranges.Count; r++)
            {
                ...
                InjectDotsAtCluster(range.ellipsisCluster, fontProvider);
            }
        }

        private void InjectDotsAtCluster(int ellipsisCluster, UniTextFontProvider fontProvider)
        {
            ...
            for (var i = 0; i < positionedCount; i++)
            {
                ...
                var dotFontId = fontProvider.FindFontForCodepoint('.');
                var dotFont = fontProvider.GetFontAsset(dotFontId);
                var dotGlyphIndex = dotFont.GetGlyphIndexForUnicode('.');
                if (dotGlyphIndex == 0) return;

                var dotLookup = dotFont.GlyphLookupTable;
                if (dotLookup == null || !dotLookup.TryGetValue(dotFont.GlyphKey(dotGlyphIndex), out var dotGlyph))
                    return;

                var upem = (float)dotFont.UnitsPerEm;
                var dotScale = uniText.CurrentFontSize * dotFont.FontScale / upem;
                var dotAdvance = dotGlyph.metrics.horizontalAdvance * dotScale;
