...
namespace LightSide
{
    ...
    public sealed class InteractiveRangeRegistry : IAttributeData
    {
        ...
        /// <summary>
        /// Calls <see cref="TextHighlighter.Update"/> on per-provider highlighters for animation updates.
        /// </summary>
        public void UpdateProviderHighlighters()
        {
            for (var i = 0; i < providers.Count; i++)
            {
                if (providers[i] is InteractiveModifier mod && mod.Highlighter != null)
                    mod.Highlighter.Update();
            }
        }
