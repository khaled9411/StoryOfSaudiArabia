...
namespace LightSide
{
    internal static unsafe class FT
    {
        ...
        [DllImport(LibraryName, CallingConvention = Cdecl)] private static extern int ut_ft_outline_decompose(IntPtr face, uint glyphIndex, float* outCurves, int* outTypes, int* outCurveCount, int maxCurves, int* outContours, int* outContourCount, int maxContours, int* outBearingX, int* outBearingY, int* outAdvanceX, int* outWidth, int* outHeight);

        [DllImport(LibraryName, CallingConvention = Cdecl)] private static extern int ut_ft_get_mm_var(IntPtr face, out IntPtr amaster);
        [DllImport(LibraryName, CallingConvention = Cdecl)] private static extern int ut_ft_done_mm_var(IntPtr library, IntPtr amaster);
        [DllImport(LibraryName, CallingConvention = Cdecl)] private static extern int ut_ft_set_var_design_coordinates(IntPtr face, uint numCoords, int* coords);
        ...
        public static void UnloadFace(IntPtr face)
        {
            if (!initialized)
            {
                Cat.MeowWarn($"[FT] UnloadFace skipped - not initialized! face={face}");
                return;
            }
            if (face == IntPtr.Zero)
            {
                Cat.MeowWarn("[FT] UnloadFace skipped - face is zero");
                return;
            }
        ...
        public static ExtendedFaceInfo GetExtendedFaceInfo(IntPtr face)
        {
            ...
            int hasOS2 = ut_ft_get_extended_face_info(face,
                ...
                out result.weightClass, out result.styleFlags);
        ...
        /// <summary>
        /// Decompose glyph outline into quadratic Bézier segments (design units).
        /// Output: 8 floats per segment (p0,ctrl,p2,unused), all segments are type=2 (quadratic).
        /// </summary>
        /// <returns>0 on success, negative on error, -2 if maxCurves exceeded, -3 if maxContours exceeded.</returns>
        public static int OutlineDecompose(IntPtr face, uint glyphIndex,
        ...
        {
            bearingX = bearingY = advanceX = width = height = 0;
            if (!initialized || face == IntPtr.Zero) return -1;
            int bx, by, ax, w, h;
            int result = ut_ft_outline_decompose(face, glyphIndex,
                outCurves, outTypes, outCurveCount, maxCurves,
                outContours, outContourCount, maxContours,
                &bx, &by, &ax, &w, &h);
            bearingX = bx; bearingY = by; advanceX = ax; width = w; height = h;
            return result;
        }
        ...
        #region Variable Font API

        /// <summary>
        /// Queries whether the font is a variable font and returns axis count.
        /// Returns 0 for non-variable fonts.
        /// </summary>
        public static int GetVariableAxisCount(IntPtr face)
        {
            if (!initialized || face == IntPtr.Zero) return 0;

            int err = ut_ft_get_mm_var(face, out var mmVar);
            if (err != 0 || mmVar == IntPtr.Zero) return 0;

            int numAxis = Marshal.ReadInt32(mmVar);
            ut_ft_done_mm_var(library, mmVar);
            return numAxis;
        }

        /// <summary>
        /// Sets variable font design coordinates.
        /// coords are in FT_Fixed format (16.16 fixed point): value * 65536.
        /// </summary>
        public static bool SetVarDesignCoordinates(IntPtr face, int* coords, int numCoords)
        {
            if (!initialized || face == IntPtr.Zero || numCoords <= 0) return false;
            return ut_ft_set_var_design_coordinates(face, (uint)numCoords, coords) == 0;
        }
