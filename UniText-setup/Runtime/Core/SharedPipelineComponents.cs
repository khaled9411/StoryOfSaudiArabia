...
namespace LightSide
{
    ...
    internal static class SharedPipelineComponents
    {
        ...
        private static readonly Lazy<Shaper> shapingEngine = new(() => Shaper.Instance);
        ...
        private static readonly Lazy<WordSegmentationProcessor> wordSegmentation = new(() =>
        {
            var processor = new WordSegmentationProcessor();
            InitializeSegmenters(processor);
            UniTextSettings.Changed += () =>
            {
                processor.Clear();
                InitializeSegmenters(processor);
            };
        ...
        /// <summary>Gets the word segmentation processor, initializing from settings on first access.</summary>
        public static WordSegmentationProcessor WordSegmentation => wordSegmentation.Value;

        private static void InitializeSegmenters(WordSegmentationProcessor processor)
        {
            var dicts = UniTextSettings.Dictionaries;
            if (dicts == null) return;

            for (var i = 0; i < dicts.Length; i++)
            {
                var dict = dicts[i];
                if (dict == null || !dict.IsValid) continue;

                try
                {
                    var trie = new DoubleArrayTrie();
                    trie.Load(dict.trieData);
                    processor.Register(new BestPathSegmenter(trie, dict.Script));
                }
                catch (Exception e)
                {
                    Debug.LogError($"Failed to load word segmentation dictionary '{dict.name}': {e.Message}");
                }
            }
        }
