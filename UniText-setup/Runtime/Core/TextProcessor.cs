...
namespace LightSide
{
    ...
    public sealed class TextProcessor
    {
        ...
        private void ApplyWordSegmentation()
        {
            ...
            if (ws == null || !ws.HasSegmenters) return;
        ...
        /// <summary>
        /// Unified font face resolution. Handles variable font axes, static variants, and synthesis
        /// in a single pass with three-tier priority:
        ///   Tier 1: Variable font axes (wght, ital, slnt from family lookup)
        ///   Tier 2: Static font face (CSS §5.2 weight matching via FontFaceLookup)
        ///   Tier 3: Synthesis (fake bold/italic — implicit, buffers remain non-zero)
        /// </summary>
        private void ResolveFontFaces()
        {
            ...
            var varAttr = buf.GetAttributeData<PooledArrayAttribute<byte>>(AttributeKeys.Variation);
            var boldAttr = buf.GetAttributeData<PooledArrayAttribute<byte>>(AttributeKeys.Bold);
            var italicAttr = buf.GetAttributeData<PooledArrayAttribute<byte>>(AttributeKeys.Italic);

            var varData = varAttr?.buffer.data;
            var boldBuf = boldAttr?.buffer.data;
            var italicBuf = italicAttr?.buffer.data;

            var hasVarConfigs = buf.variationConfigs.count > 0 && varData != null;
            var hasBoldData = boldBuf != null;
            var hasItalicData = italicBuf != null;

            if (!hasVarConfigs && !hasBoldData && !hasItalicData) return;
            ...
            int cachedOverrideFontId = 0;
            bool cachedHasOverride = false;
            bool cachedClearBold = false, cachedClearItalic = false;
            bool cachedNeedsGlyphCheck = false;
            UniTextFont cachedGlyphCheckFont = null;
            bool hasCached = false;

            for (var i = 0; i < cpCount; i++)
            {
                var varByte = hasVarConfigs && (uint)i < (uint)varData.Length ? varData[i] : (byte)0;
                var boldByte = hasBoldData && (uint)i < (uint)boldBuf.Length ? boldBuf[i] : (byte)0;
                var italicByte = hasItalicData && (uint)i < (uint)italicBuf.Length ? italicBuf[i] : (byte)0;

                if (varByte == 0 && boldByte == 0 && italicByte == 0) continue;
                ...
                {
                    if (!cachedHasOverride) continue;

                    if (cachedNeedsGlyphCheck && Shaper.GetGlyphIndex(cachedGlyphCheckFont, (uint)cp) == 0)
                        continue;

                    EnsureFontIdOverrides(cpCount);
                    fontIdOverrides.data[i] = cachedOverrideFontId;
                    if (cachedClearBold) boldBuf[i] = 0;
                    if (cachedClearItalic) italicBuf[i] = 0;
                    continue;
                }

                prevFontId = fontId;
                prevVarByte = varByte;
                prevBoldByte = boldByte;
                prevItalicByte = italicByte;
                hasCached = true;
                cachedHasOverride = false;
                cachedClearBold = false;
                cachedClearItalic = false;
                cachedNeedsGlyphCheck = false;
                cachedGlyphCheckFont = null;

                var familyIndex = fp.GetFamilyIndex(fontId);
                ref var lookup = ref fp.GetFamilyLookup(familyIndex);

                var wantItalic = italicByte != 0;
                var varFont = wantItalic
                    ? (lookup.variableItalic ?? lookup.variableUpright)
                    : lookup.variableUpright;

                if (varFont != null)
                {
                    var fontAxes = varFont.VariableAxes;
                    if (fontAxes != null && fontAxes.Length > 0)
                    {
                        bool hasWght = false, hasItal = false, hasSlnt = false;
                        for (int a = 0; a < fontAxes.Length; a++)
                        {
                            var tag = fontAxes[a].tag;
                            if (tag == VariationModifier.axisTags[0]) hasWght = true;
                            else if (tag == VariationModifier.axisTags[2]) hasItal = true;
                            else if (tag == VariationModifier.axisTags[3]) hasSlnt = true;
                        }

                        var mergedConfig = varByte > 0
                            ? buf.variationConfigs.data[varByte - 1]
                            : default(VariationModifier.VariationConfig);

                        bool axisHandlesBold = false;
                        bool axisHandlesItalic = false;

                        if (boldByte != 0 && hasWght)
                        {
                            var cssWeight = BoldModifier.DecodeCssWeight(boldByte);
                            mergedConfig.wght = new VariationModifier.AxisValue
                                { value = cssWeight, mode = VariationModifier.ValueMode.Absolute };
                            mergedConfig.mask |= VariationModifier.AxisMask.Wght;
                            axisHandlesBold = true;
                        }

                        if (italicByte != 0 && hasItal)
                        {
                            mergedConfig.ital = new VariationModifier.AxisValue
                                { value = 1f, mode = VariationModifier.ValueMode.Absolute };
                            mergedConfig.mask |= VariationModifier.AxisMask.Ital;
                            axisHandlesItalic = true;
                        }
                        else if (italicByte != 0 && hasSlnt)
                        {
                            mergedConfig.slnt = new VariationModifier.AxisValue
                                { value = -12f, mode = VariationModifier.ValueMode.Absolute };
                            mergedConfig.mask |= VariationModifier.AxisMask.Slnt;
                            axisHandlesItalic = true;
                        }

                        if (!axisHandlesItalic && italicByte != 0 && varFont.FaceInfo.isItalic)
                            axisHandlesItalic = true;

                        var varFontId2 = UniTextFontProvider.GetFontId(varFont);
                        bool switchingFont = varFontId2 != fontId;

                        if (mergedConfig.mask != VariationModifier.AxisMask.None)
                        {
                            var axisValues = VariationModifier.ResolveAxes(in mergedConfig, fontAxes,
                                out var hbVariations, out var ftCoords);

                            var varHash48 = varFont.ComputeVarHash48(axisValues);
                            var varFontId = (int)varHash48;
                            if (varFontId == fontId) varFontId ^= unchecked((int)0x80000000);

                            variationMap ??= new Dictionary<int, VariationRunInfo>();
                            if (!variationMap.ContainsKey(varFontId))
                            {
                                variationMap[varFontId] = new VariationRunInfo
                                {
                                    baseFontHash = varFont.FontDataHash,
                                    varHash48 = varHash48,
                                    hbVariations = hbVariations,
                                    ftCoords = ftCoords,
                                };
                                fp.RegisterFontAsset(varFontId, varFont);
                            }

                            EnsureFontIdOverrides(cpCount);
                            fontIdOverrides.data[i] = varFontId;
                            cachedOverrideFontId = varFontId;
                            cachedHasOverride = true;
                            cachedClearBold = axisHandlesBold;
                            cachedClearItalic = axisHandlesItalic;
                            if (axisHandlesBold) boldBuf[i] = 0;
                            if (axisHandlesItalic) italicBuf[i] = 0;
                            continue;
                        }

                        if (switchingFont)
                        {
                            fp.RegisterFontAsset(varFontId2, varFont);
                            EnsureFontIdOverrides(cpCount);
                            fontIdOverrides.data[i] = varFontId2;
                            cachedOverrideFontId = varFontId2;
                            cachedHasOverride = true;
                            cachedClearBold = false;
                            cachedClearItalic = axisHandlesItalic;
                            if (axisHandlesItalic && hasItalicData) italicBuf[i] = 0;
                            continue;
                        }
                    }
                }

                if (lookup.hasFaces && (boldByte != 0 || italicByte != 0))
                {
                    var font = fp.GetFontAsset(fontId);
                    var baseWeight = font.FaceInfo.weightClass;
                    var targetWeight = boldByte != 0 ? BoldModifier.DecodeCssWeight(boldByte) : baseWeight;

                    var face = lookup.FindFace(targetWeight, wantItalic);

                    if (face == null && boldByte != 0 && italicByte != 0)
                        face = lookup.FindFace(targetWeight, false);

                    if (face != null && UniTextFontProvider.GetFontId(face) == fontId)
                        face = null;

                    if (face != null && Shaper.GetGlyphIndex(face, (uint)cp) == 0)
                        face = null;

                    if (face != null)
                    {
                        var faceFontId = UniTextFontProvider.GetFontId(face);
                        fp.RegisterFontAsset(faceFontId, face);

                        EnsureFontIdOverrides(cpCount);
                        fontIdOverrides.data[i] = faceFontId;
                        cachedOverrideFontId = faceFontId;
                        cachedHasOverride = true;
                        cachedNeedsGlyphCheck = true;
                        cachedGlyphCheckFont = face;

                        var clearBold = boldByte != 0;
                        var clearItalic = italicByte != 0 && face.FaceInfo.isItalic;
                        cachedClearBold = clearBold;
                        cachedClearItalic = clearItalic;
                        if (clearBold) boldBuf[i] = 0;
                        if (clearItalic) italicBuf[i] = 0;
                        continue;
                    }
                }
            }
        ...
        private void EnsureFontIdOverrides(int cpCount)
        {
            if (hasFontIdOverrides) return;
        ...
        private int GetFontIdForCluster(Span<int> cpSpan, int start, int end, UniTextFontProvider fp)
        {
            if (hasFontIdOverrides)
            {
                ...
                if ((uint)start < (uint)overrides.Length && overrides[start] != 0)
        ...
        private void Shape()
        {
            ...
            var scAttr = buf.GetAttributeData<PooledArrayAttribute<byte>>(AttributeKeys.SmallCaps);
            byte[] scBuf = scAttr?.buffer.data;

            var supAttr = buf.GetAttributeData<PooledArrayAttribute<sbyte>>(AttributeKeys.ScriptPosition);
            sbyte[] supBuf = supAttr?.buffer.data;

            buf.shapedGlyphs.EnsureCapacity(cpCount);
        ...
        private static int CollectRunFeatures(byte[] scBuf, sbyte[] supBuf, int runStart, int runLen)
        {
            ...
            if (scBuf != null)
            {
                ...
                {
                    if ((uint)c < (uint)scBuf.Length && scBuf[c] == 1)
                    {
                        featureScratch[count++] = new HB.hb_feature_t
                        {
                            tag = Shaper.SmcpTag, value = 1,
            ...
            if (supBuf != null)
            {
                CollectRangeFeatures(supBuf, runStart, runEnd, 1, Shaper.SupsTag, ref count);
                CollectRangeFeatures(supBuf, runStart, runEnd, -1, Shaper.SubsTag, ref count);
            }

            return count;
        }

        private static void CollectRangeFeatures(sbyte[] buf, int runStart, int runEnd, sbyte nativeMode, uint tag, ref int count)
        {
            var i = runStart;
            while (i < runEnd)
            {
                while (i < runEnd && ((uint)i >= (uint)buf.Length || buf[i] != nativeMode))
                    i++;
                if (i >= runEnd) break;
                ...
                {
                    tag = tag, value = 1,
                    start = (uint)(rangeStart - runStart),
                    end = (uint)(i - runStart)
                };
            }
        }
            ...
            LeadingDistribution distribution = LeadingDistribution.HalfLeading)
        {
            ...
            var globalMaxDescDepth = -mainDescender;
            var globalMaxAscHeight = mainAscender;
            if (fontProvider != null)
            {
                var shapedRuns = buf.shapedRuns.Span;
                var lastCheckedFontId = -1;
                for (var r = 0; r < shapedRuns.Length; r++)
                {
                    var fontId = shapedRuns[r].fontId;
                    if (fontId == lastCheckedFontId) continue;
                    lastCheckedFontId = fontId;

                    var font = fontProvider.GetFontAsset(fontId);
                    if (font is null || font.FontScale == 1f) continue;
                    ...
                    if (asc > globalMaxAscHeight) globalMaxAscHeight = asc;
                    if (-desc > globalMaxDescDepth) globalMaxDescDepth = -desc;
                }
            }

            var globalMinAdvance = globalMaxDescDepth + globalMaxAscHeight + lineSpacing;
            ...
            for (var i = 0; i < lineCount - 1; i++)
            {
                ...
                if (globalMinAdvance > advance)
                    advance = globalMinAdvance;
