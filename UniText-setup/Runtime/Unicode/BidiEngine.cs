    ...
    internal sealed class BidiEngine
    {
        ...
        private BidiResult ProcessInternal(ReadOnlySpan<int> codePoints, byte? forcedParagraphLevel)
        {
            ...
            for (var pIndex = 0; pIndex < paragraphCount; pIndex++)
            {
                ...
                ResolvePairedBracketsForParagraph(codePoints, sequences, bidiClasses);
            ...
            BidiClass[] bidiClasses)
        {
            ...
            var pairsData = bracketPairs.buffer.data;
            var pairsLen = bracketPairs.Count;
            for (var j = 1; j < pairsLen; j++)
            {
                var key = pairsData[j];
                var k = j - 1;
                while (k >= 0 && pairsData[k].openIndex > key.openIndex)
                {
                    pairsData[k + 1] = pairsData[k];
                    k--;
                }
                pairsData[k + 1] = key;
            }
