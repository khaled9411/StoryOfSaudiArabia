...
namespace LightSide
{
    internal static class StyledListUtility
    {
        internal struct ListCallbacks
        {
            ...
            public Action<Rect, SerializedProperty> onAddButtonClicked;
        }
        ...
        private static void DrawHeader(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            {
                if (activeCallbacks.onAddButtonClicked != null)
                {
                    activeCallbacks.onAddButtonClicked(addButtonRect, property);
                }
                else
                {
                    int newIndex = property.arraySize;
                    property.arraySize++;
                    ClearNewElement(property.GetArrayElementAtIndex(newIndex));
        ...
        internal static void ApplyToAllTargets(SerializedObject so, string path,
            string undoLabel, Action<SerializedProperty> operation)
        {
            foreach (var target in so.targetObjects)
            {
                Undo.RecordObject(target, undoLabel);
                var targetSo = new SerializedObject(target);
                var prop = targetSo.FindProperty(path);
        ...
        private static void ShowHeaderContextMenu(SerializedProperty property)
        {
            ...
            if (property.arraySize > 0)
            {
                menu.AddItem(new GUIContent("Copy List"), false, () =>
                {
                    ...
                    if (prop != null) PropertyClipboard.CopyArray(prop);
                });
            }
            else
            {
                menu.AddDisabledItem(new GUIContent("Copy List"));
            }

            if (PropertyClipboard.CanPasteArray(property))
            {
                var count = PropertyClipboard.ArrayClipCount;
                menu.AddItem(new GUIContent($"Paste List ({count} items)"), false, () =>
                    ApplyToAllTargets(so, path, "Paste List", PropertyClipboard.PasteArray));
            }
            else
            {
            ...
            if (PropertyClipboard.CanPasteElementIntoArray(property))
            {
                var clipLabel = PropertyClipboard.GetElementClipLabel();
                menu.AddItem(new GUIContent($"Paste Element ({clipLabel})"), false, () =>
                    ApplyToAllTargets(so, path, "Paste Element", PropertyClipboard.PasteElementIntoArray));
            }
            else
            {
            ...
            if (property.arraySize > 0)
            {
                menu.AddItem(new GUIContent("Clear"), false, () =>
                    ApplyToAllTargets(so, path, "Clear List", p => p.ClearArray()));
            }
            else
            {
        ...
        private static void ShowElementContextMenu(SerializedProperty arrayProperty, int index)
        {
            var menu = new GenericMenu();
            ...
            menu.AddItem(new GUIContent("Copy"), false, () =>
            {
                ...
                if (prop != null && index < prop.arraySize)
                    PropertyClipboard.CopyElement(prop.GetArrayElementAtIndex(index));
            });

            if (PropertyClipboard.CanPasteElement(arrayProperty.GetArrayElementAtIndex(index)))
            {
                var clipLabel = PropertyClipboard.GetElementClipLabel();
                menu.AddItem(new GUIContent($"Paste ({clipLabel})"), false, () =>
                    ApplyToAllTargets(so, path, "Paste Element", p =>
                    {
                        if (index >= p.arraySize) return;
                        PropertyClipboard.PasteElement(p.GetArrayElementAtIndex(index));
                    }));
            }
            else
            {
            ...
            menu.AddItem(new GUIContent("Duplicate"), false, () =>
                ApplyToAllTargets(so, path, "Duplicate Element", p =>
                {
                    if (index >= p.arraySize) return;
                    PropertyClipboard.DuplicateElement(p, index);
                }));

            menu.AddSeparator("");

            menu.AddItem(new GUIContent("Delete"), false, () =>
                ApplyToAllTargets(so, path, "Delete Element", p =>
                {
                    if (index >= p.arraySize) return;
        ...
        private static void ClearNewElement(SerializedProperty element)
        {
            ...
            switch (evt.type)
            {
                ...
                case EventType.MouseUp:
                    if (dragPropertyPath == propertyPath && dragIndex >= 0)
                    {
                        ...
                        {
                            int finalTargetIndex = targetIndex > dragIndex ? targetIndex - 1 : targetIndex;
        ...
        private static void DrawElement(Rect rect, SerializedProperty arrayProperty, SerializedProperty element, int index)
        {
            ...
            {
                var headerRow = new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight + ElementPadding);
                if (headerRow.Contains(Event.current.mousePosition))
                {
                    ShowElementContextMenu(arrayProperty, index);
                    Event.current.Use();
                }
            }
